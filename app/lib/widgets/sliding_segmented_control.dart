...
class _SegmentedControlState<T> extends State<CupertinoSlidingSegmentedControl<T>>
  ...
  set pressed(T newValue) {
    ...
    if (_pressed != null) {
      _pressControllers[_pressed]
          ?.animateTo(0, duration: _kOpacityAnimationDuration, curve: Curves.ease);
    }
    if (newValue != _highlighted && newValue != null) {
      _pressControllers[newValue]
          .animateTo(1, duration: _kOpacityAnimationDuration, curve: Curves.ease);
    }
    _pressed = newValue;
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<AnimationController> controllers = _highlightControllers.values;
    controllers.addAll(_pressControllers.values);
...
class _RenderSegmentedControl<T> extends RenderBox
  ...
  int indexFromLocation(Offset location) {
    return childCount == 0
        ...
        : ((location.dx / (size.width / childCount)).floor().clamp(0, childCount - 1) as int);
  }
  ...
  void _playThumbScaleAnimation({@required bool isExpanding}) {
    ...
    _thumbScaleTween =
        Tween<double>(begin: currentThumbScale, end: isExpanding ? 1 : _kMinThumbScale);
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (highlightedIndex != null) {
      ...
      final Rect unscaledThumbTargetRect =
      ...
      if (_needsThumbAnimationUpdate) {
        ...
        _currentThumbTween = RectTween(
            begin: currentThumbRect ?? unscaledThumbTargetRect, end: unscaledThumbTargetRect);
        ...
        _currentThumbTween =
            RectTween(begin: _currentThumbTween.begin, end: unscaledThumbTargetRect);
      }
      ...
      currentThumbRect =
          _currentThumbTween?.evaluate(state.thumbController) ?? unscaledThumbTargetRect;
  ...
  void _paintSeparator(PaintingContext context, Offset offset, RenderBox child) {
    ...
    final double opacity =
        manifest?.separatorTween?.evaluate(state.separatorOpacityController) ?? 1;
    ...
    while (child != null) {
      ...
      if ((childParentData.offset & child.size).contains(position)) {
        ...
        return result.addWithRawTransform(
          Matrix4: MatrixUtils.forceToPoint(center),
        );
      }
